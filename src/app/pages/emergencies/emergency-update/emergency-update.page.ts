...
export class EmergencyUpdatePage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.sub1 = this.infoService.getEmergencyInfoById(id)
    .subscribe(info => {
      if (!info) {
        this.router.navigate(['/tabs/tabs/emergency-home']);
      } else {
        this.info = info;

        this.updateInfoForm = new FormGroup({
          'accidents': new FormControl(this.info.accidents),
          'introduction': new FormControl(this.info.introduction),
          'methods': new FormControl(this.info.methods),
  ...
  updateInfo(values: any) {
    ...
    let updatedInfo: EmergencyInfo = { id: this.info.id, ...values };
    this.infoService.updateEmergencyInfo(updatedInfo);
  }
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: () => {
            ...
            this.infoService.deleteEmergencyInfo(infoId).then(
              res => this.router.navigate(['/tabs/tabs/emergency-home'])
            );
          }
        }
      ]
